<?php
function getAdminUserTable($users) {
	if(empty($users)) {
	...
	foreach($users as $index => $user) {
		// Check controls
		if(function_exists('getAdminUserControls')) {
			$controls = getAdminUserControls($user,$context);
			$controls = <<<EOT
...
EOT;
	...
	$markup = <<<EOT
<table id="{$context}" class="userTable ui-block ui-table ui-items ui-checkbox-group">
<thead>
<tr>
	$checkbox_switch
