...
(function($){
	...
	jQuery.fn.statusBox = function(options) {
		this.each(function(index) {
			var $this = jQuery(this);
			var e = $this.find('div.ui-form-status');
			if( e.length > 0 ) {
				e.find('.message').html(options.message);
				if(options.type == 'error') e.addClass('error');
				else e.removeClass('error');
			} else {
				var e = jQuery('<div class="ui-form-status"><div class="inner"><div class="message">'+options.message+'</div><div class="close"><span>닫기</span></div></div>');
				if(options.type == 'error') e.addClass('error');
				else e.removeClass('error');
				$this.append(e);
				...
				if($this.css('position') == 'relative') {
					e.css({
						'top' : parseInt( ( $this.outerHeight() - e.outerHeight() ) / 2 )+'px',
						'left' : parseInt( ( $this.outerWidth() - e.outerWidth() ) / 2 )+'px',
					});
				} else {
					e.css({
						'top' : ( $this.position().top + parseInt( ( $this.outerHeight() - e.outerHeight() ) / 2 ) )+'px',
