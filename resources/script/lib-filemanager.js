//-----------------------------------------------------------------------------
//	This file is a modified copy:
//		(original) contributes/filemanager/filemanager/js/include.min.js
//		(modified) resources/script/lib-filemanager.js
//-----------------------------------------------------------------------------
var version="9.4.0";
...
function taogi_toggle_select(file,external,trigger){
	if(jQuery('#popup').val()==1) {
		var container=window.opener;
	} else {
		var container=window.parent;
	}
	var target = jQuery('#'+external,container.document).closest('slide-item');
    ...
    if (external!=""){
		var item = trigger.closest('.file');
		item.attr('data-filepath',base_url+path+file);
		item.toggleClass('taogi-selected');
    } else {
		apply_any(base_url+path, file);
	}
}
jQuery(document).ready(function(e){
	jQuery('.button.taogi-send-selected').on('click',function(e){
		if(jQuery('#popup').val()==1) {
			var container=window.opener;
		} else {
			var container=window.parent;
		}
		var target = jQuery('#'+jQuery(this).attr('data-field_id'),container.document);
		var items = [];

		jQuery('.taogi-selected').each(function(index){
			var item = jQuery(this);
			items.push(item.attr('data-filepath'));
		});
