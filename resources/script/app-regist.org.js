...
*/
	...
	var params = "email_id="+f1.val()+"&email_id_confirm="+f2.val()+"&name="+n.val()+"&taoginame="+tn.val()+"&password="+p1.val()+"&password_confirm="+p2.val()+"&requestURI="+jQuery(TheForm).find('input[name="requestURI"]').val()+"&join_type="+jQuery(TheForm).find('input[name="join_type"]').val();
	jQuery.ajax({
		type: 'POST',
		url: url,
		data: params,
...
function init_keyEvent() {
	...
	jQuery('.button.check_dup').click(function(e) {
		...
		jQuery.ajax({
			...
			dataType: 'json',
			beforeSend: function() {
				jfe_Block_onRequest('따오기 고유주소 중복 검사중입니다. 잠시만 기다려주세요.');
			},
			success: function(json) {
				jfe_unBlock_afterRequest();
				var error = parseInt(json.error);
